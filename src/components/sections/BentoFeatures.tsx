...
const FeatureDecorations = React.lazy(() => import('../three/FeatureDecorations'));
  ...
  return (
    <article
      className={`
        ...
        transition-all duration-500 var(--ease-spring) card-hover
        ${isActive
    ...
    >
      <div className={`p-6 lg:p-8 h-full flex flex-col transition-all duration-300 ${isActive ? 'justify-between' : 'justify-start'}`}>
        ...
        {isActive && feature.stat && (
          <div className="mt-auto pt-6 flex items-end gap-3 animate-fade-in-up" style={{ animationDelay: '150ms' }}>
...
const AccordionItem: React.FC<AccordionItemProps> = React.memo(({ feature, isOpen, onToggle, index }) => {
  ...
  return (
    <article
      className={`
        group relative bg-white/60 border-b border-noir/5 last:border-b-0 overflow-hidden cursor-pointer
        transition-all duration-300 var(--ease-apple) card-hover
        ${isOpen ? 'bg-gradient-to-br ' + feature.gradient : 'hover:bg-white/80'}
      `}
...
const BentoFeatures: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      <div ref={containerRef} className={`relative z-10 max-w-7xl mx-auto transition-all duration-700 ease-out ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        {/* Section Header */}
